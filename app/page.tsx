...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/data/years/index.json", { cache: "no-store" })
      ...
      .then((data: { years: number[] }) => { setYears(data.years); setYear(data.years[0] ?? null); });
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`/data/years/${year}.json`, { cache: "no-store" })
      ...
      .then(async (data: StormYear) => {
        ...
        const defaultBasin: Basin = storms.some((storm) => storm.basin === "WP") ? "WP" : "all";
        setStormYear({ ...data, storms });
        setBasin(defaultBasin);
        setSelectedId(storms.find((storm) => defaultBasin === "all" || storm.basin === defaultBasin)?.id ?? null);
      });
  }, [year]);
